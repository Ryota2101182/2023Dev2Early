...
<div class="container">
	<div class="row">
		...
		<?php
		$inventorycheck1 = "SELECT * FROM inventories WHERE item_id=? AND inventory_itemsize='S'";
		$ps = $pdo->prepare($inventorycheck1);
		$ps->bindValue(1,$row['item_id'],PDO::PARAM_INT);
		$ps->execute();
		$S_check = $ps->fetch();
		
		$inventorycheck2 = "SELECT * FROM inventories WHERE item_id=? AND inventory_itemsize='M'";
		...
		if($S_check['inventory_sum'] == 0 and $M_check['inventory_sum'] == 0){
			echo "SOLD OUT";
		}else{
